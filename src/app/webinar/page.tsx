...
export default function WebinarsPage() {
  ...
  const [activeWebinarPlayer, setActiveWebinarPlayer] = useState<Webinar | null>(null);
  ...
  useEffect(() => {
    ...
    const loadWebinars = async () => {
      try {
        ...
        const response = await fetch("https://api.theschoolofoptions.com/api/v1/zoom/webinar-list");
  ...
  const joinWebinar = useCallback(
    (webinar: Webinar) => {
      ...
      console.log('[DEBUG] Joining webinar with data:', webinar);
      console.log('[DEBUG] Webinar fields:', {
        id: webinar.id,
        uuid: webinar.uuid,
        type: webinar.type,
      ...
      setError(null);
      setActiveWebinarPlayer(webinar);
    },
    [isWebinarLive, isAuthenticated]
  );
  ...
  const handleCloseWebinarPlayer = useCallback(() => {
    setActiveWebinarPlayer(null);
  }, []);

  const handleWebinarPlayerError = useCallback((errorMessage: string) => {
  ...
  return (
    <div className="min-h-screen bg-white">
                  ...
                  {liveWebinars.map((webinar) => (
                    <div key={webinar.id} className="bg-white rounded-2xl p-6 shadow-lg border-2 border-accent/20 hover:shadow-xl transition-all duration-300">
                      ...
                      <button
                        ...
                        className="w-full bg-accent hover:bg-accent/90 text-navy px-6 py-3 rounded-lg font-semibold transition-colors flex items-center justify-center"
                      >
                        <PlayIcon className="h-5 w-5 mr-2" />
                        {isAuthenticated ? "Join Live" : "Sign In to Join"}
                      </button>
                    </div>
                  ))}
                </div>
      ...
      {/* Enhanced Webinar Player */}
      {activeWebinarPlayer && (
        <ZoomWebinarPlayer
          webinarId={activeWebinarPlayer.join_url || activeWebinarPlayer.id}
          webinarTitle={activeWebinarPlayer.topic}
          userName={user?.fullName || "Guest"}
          userEmail={user?.email || "guest@example.com"}
          onClose={handleCloseWebinarPlayer}
          onError={handleWebinarPlayerError}
        />
      )}
